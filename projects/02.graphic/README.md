...
- a 태그를 모드 변경해주고, background-image에 사용된 gif도 변경해주어 느낌을 바꾸어주었습니다.
- 모바일 사이즈일때는 nav바가 아래로 가면서 배경색이 생긴다. 즉, pc모드일때랑 mobile일떄 background-color가 차이나는데, resize를 통해 버튼이 눌른 상태에서 화면이 작아져서 mobile 모드가 되어도 원하는 색이 나오도록 JS와 CSS를 처리해 주었습니다.
